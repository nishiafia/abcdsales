              ...
              <div class="card-header">
                <h3 class="card-title">Purchase Order Detail</h3>
                  ...
                  <div class="card-tools">
                    <router-link :to="`/purchaseorder/${porder.id}`" class="btn btn-success" data-id="porder.id"   title="Edit"  @click.native="editModalWindowOrder(porder)">
                ...
                <table class="table table-hover">
                  <tbody  >
                    ...
                    <tr>
                        ...
                        <td> {{porder.vendordata.customername}}
                        
                        </td>
                    </tr>
                    <tr>
                        <td class="tabletitle">Date :</td> 
                        <td>  {{porder.currentdate | formatDate}}</td> 
                    </tr>
                    <tr>
                    ...
                    <tr v-if="porder.duedeliverydate !== ''">
                        ...
                        <td> {{porder.duedeliverydate | formatDate}}</td> 
                    </tr>
                     <tr v-if="porder.refpo !== 0">
                        <td class="tabletitle">Reference PO :</td> 
                        <td> 
                           {{porder.refporder.ordernumber}}</td> 
                    </tr>
                     <tr>
                        ...
                        <td>  {{porder.notes}}
                          ...
                          <table class="table table-hover itemtable">
                            ...
                            <td v-if="pitem.delivered === 0">
                              ...
                              <td> 
                              <router-link :to="`/purchaseorder/${pitem.orderid}`" data-id="pitem.id"  title="Edit"  @click.native="editModalWindow(pitem)">
                              <i class="fa fa-edit blue"></i>
                              </router-link>

                              
                                <router-link v-if="pitem.delivered === 1" :to="`/purchaseorder/${pitem.orderid}`" data-id="pitem.id"  title="Return"  @click.native="returnItem(pitem.id)">
                                <i class="fa fa-get-pocket" aria-hidden="true"></i>
                                </router-link>

                                <router-link v-if="pitem.delivered === 0 && porder.dueamount >= pitem.total" :to="`/purchaseorder/${pitem.orderid}`"   data-id="pitem.id"   title="Cancel"  @click.native="cancelItem(pitem.id)">
                              ...
                              <tr class="multirow" v-if="porder.paidamount !== 0">
                               ...
                               <td>Action</td>
                                ...
                                <td v-if="paccount.paymentmethod === 1">
                                  <a v-bind:href="'/#/purchaseorder/'+porder.id" title="show accounts details" data-id="paccount.id" class="methodlink" @click="showModalWindowpayment(paccount.id,paccount.paymentmethod)">
                                  Cash
                                  </a>
                                  </td>
                                  ...
                                  <td>{{ paccount.payamount | toCurrency}} </td>
                                  <td>{{ paccount.paymentdate | formatDate}}</td>
                                  <td> 
                                    <router-link :to="`/purchaseorder/${paccount.orderid}`" data-id="paccount.id"  title="Edit"  @click.native="editModalWindowPayment(paccount)">
                                    <i class="fa fa-edit blue"></i>
                                    </router-link>
                                     |
                                    <router-link  :to="`/purchaseorder/${paccount.orderid}`" data-id="paccount.id"  title="Delete"  @click.native="deletePayment(paccount.id)">
                           ...
                           <div class="modal fade" id="addNewpayment" tabindex="-1" role="dialog" aria-labelledby="addNewPayment" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered" role="document">
                              <div class="modal-content" >
                                ...
                                <form @submit.prevent="editMode1 ? updatePurchaseaccount() :createPurchaseaccount()" method="post">
                                ...
                                <td colspan="2">
                               <label for="windows">
                                  <input type="radio" id="paymentmethodCash" name="paymentmethod" value="1" v-model="form.paymentmethod" v-on:change="showCash" >
                                  Cash
                                  </label>
                                  <label for="windows">
                                  <input type="radio" id="paymentmethodBkash" name="paymentmethod" value="2" v-model="form.paymentmethod" v-on:change="showBkash" >
                                  Bkash
                                  </label>
                                   <label for="windows">
                                  <input type="radio" id="paymentmethodBank" name="paymentmethod" value="3" v-model="form.paymentmethod" v-on:change="showBank" >
                                  Bank
                                  </label>
                               ...
                               <tr>
                                 ...
                                 <td>
                                    ...
                                    <input type="text" v-model="form.payamount" name="payamount" id="payamount" :class="{ 'is-invalid': form.errors.has('payamount') }"  @keypress="onlyNumber" @change="amountvalidation(porder.dueamount)" required />
                              ...
                              <div class="modal-content" >
                               ...
                               <tr>
                                ...
                                <td >
                                   ...
                                   <!--select  name="variationsid1" v-model="formitem.variationsid"  class="form-control"  multiple="true"  required>
                                  <option v-for="varl in variationlist" v-bind:value="varl.variationid" :key="varl.variationid">
                     ...
                     <tr>
                        ...
                        <td colspan="2" class="detailbottom"><label for="windows" class="btn btn-success">
                                  <input type="radio" id="orderdelivered" name="orderdelivered" value="1"  v-model="orderdelivered" :disabled="deliverybutton"   v-on:change="deliveryOrder(porder.id)" >
                                  Confirm Delivered
                                  </label>
                                  <label for="windows" class="btn btn-danger">
                                  <input type="radio" id="orderdelivered" name="orderdelivered" value="2" v-model="orderdelivered" :disabled="returnbutton"  v-on:change="returnOrder(porder.id)" >
                                  Return Order 
                                  </label>
                                  <label for="windows" class="btn btn-danger" v-if="porder.paidamount === 0.00">
                                  <input type="radio" id="orderdelivered" name="orderdelivered" value="3" v-model="orderdelivered" :disabled="returnbutton"  v-on:change="cancelOrder(porder.id)" >
                                  Cancel Order 
                                  </label>
    ...
    export default {
        ...
        showModalWindowpayment(accountid,methodid){
          ...
          if(methodid == 3){
           ...
           axios.get("/getpaymentmethodinformation/"+accountid, {headers})
          .then( data =>{
         ...
         showBkash: function() {
          ...
          amountvalidation(dueamt){
            console.log("forminput=", this.form.payamount);
             console.log("due=", dueamt);
              this.errors = [];
             if(this.form.payamount>dueamt)
             {
          ...
          checkpurchaseItem(event){
            ...
            .then( response =>{
             this.formitem.price = (parseFloat(response.data[0].productcost).toFixed(2));
             pricemultiply=parseFloat(this.formitem.price)*this.formitem.qty;
          ...
          checkItem(){
            ...
            this.$Progress.start()
            axios.post('/updateItemDelivery',{itemids: this.checkedItems,porderid:this.poid})
             .then((response) => {
                console.log("response:",response.data);
              if(response.data === '')
              {  
              Fire.$emit('AfterCreatedPurchaseorderdetailLoadIt'); //custom events
              Toast.fire({
              icon: 'error',
              title: 'Not Done!!'
              })
          ...
          openModalWindow(id,dueamt){
            ...
            this.form.payamount=parseFloat(dueamt);
      ...
      loadDiscount() {
              ...
              form.put('api/purchaseaccount/'+this.form.id)
               .then(()=>{
                   ...
                   Toast.fire({
                    ...
                    Fire.$emit('AfterCreatedPurchaseorderdetailLoadIt');
        ...
        created() { //Like Mounted this method
                ...
                Fire.$on('AfterCreatedPurchaseorderdetailLoadIt',()=>{ //custom events fire on
                this.loadPurchaseorder();
